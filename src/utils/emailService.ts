...
export const generateInvoicePDF = (invoice: InvoiceData): Uint8Array => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(20);
    doc.text(`Factura Nº: ${invoice.num_factura}`, 14, 20);
    ...
    // --- TABLA DATOS CLIENTE ---
    const clientData = [
        ["Nombre Completo/Razón Social:", invoice.nombre_completo],
        ["CIF:", invoice.cif || "---"],
        ["Dirección de facturación:", invoice.direccion || "---"],
    ...
    // --- TABLA CONTRATO (RESERVA) ---
    const reservationData = [
        ["Marca, Modelo y Color:", invoice.coche],
        ["Matrícula:", invoice.matricula],
        ["Tipo de Plaza:", invoice.tipo_plaza],
        ["Fecha Entrada:", `${formatearFecha(invoice.fecha_entrada)} ${invoice.hora_entrada || ""}`],
        ["Fecha Salida:", `${formatearFecha(invoice.fecha_salida)} ${invoice.hora_salida || ""}`],
        ["Forma de Pago:", invoice.metodo_pago]
    ];
